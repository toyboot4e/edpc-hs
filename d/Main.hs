#!/usr/bin/env stack
{- stack script
--resolver lts-16.11 --package array --package bytestring --package containers
...
main = do
  ...
  input <- replicateM n getLineIntList

  let dp = foldl' step s0 input
      s0 = [(0, 0)] :: [(Int, Int)]
      step wvs [w, v] =
        -- REMARK: start with `-1` so that we never delete `(0, 0)` on `merge`
        merge (-1) wvs $ filter ((<= wLimit) . fst) $ map (bimap (+ w) (+ v)) wvs
      step _ _ = error "unreachabele"
      merge :: Int -> [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)]
      merge _ [] ys = ys
      merge _ xs [] = xs
      merge maxV xs@(x : xrest) ys@(y : yrest)
        -- REMARK: we HAVE TO remove useless searches
  ...
  -- let !_ = traceShow (wLimit, length dp) ()
  -- let !_ = traceShow dp ()
  print $ maximum $ map snd dp
