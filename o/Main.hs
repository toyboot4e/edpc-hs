...
main :: IO ()
main = do
  [nPairs] <- getLineIntList
  !input <- V.replicateM nPairs (map fst . filter snd . zip [(0 :: Int)..] . map (== 1) <$> getLineIntList)

  let !modulus = 1_000_000_007 :: Int
      addMod :: Int -> Int -> Int
      addMod !c1 !c2 = (c1 + c2) `rem` modulus

  -- Sparse: TLE
  -- let !result = V.foldl' step s0 input
  --     !s0 = IM.singleton 0 (1 :: Int)
  --     step !im0 !bits = foldl' mergeBit IM.empty bits
  --       where
  --         mergeBit :: IM.IntMap Int -> Int -> IM.IntMap Int
  --         mergeBit !acc0 !nBit =
  --           let !bcs = map (first (bit nBit .|.)) $ filter (\(!bs, !_) -> not $ testBit bs nBit) $ IM.toList im0
  --               innerStep :: IM.IntMap Int -> (Int, Int) -> IM.IntMap Int
  --               innerStep !im' (!bs, !c) = IM.insertWith addMod bs c im'
  --            in foldl' innerStep acc0 bcs

  -- Dense
  let !n = shift 1 nPairs :: Int
  let !result = VU.create $ do
        !dp <- VUM.replicate (succ n) (0 :: Int)
        VUM.write dp 0 1

        forM_ [0 .. pred (pred n)] $ \ !x -> do
          !c <- VUM.read dp x
          when (c > 0) $ do
            let !nBits = popCount x
            -- let !_ = traceShow (x, c, nBits) ()
            forM_ (filter (\ !i -> not (testBit x i)) $ input V.! nBits) $ \ !nBit -> do
                let !x' = x .|. bit nBit
                VUM.modify dp (addMod c) x'

        return dp

  -- let !_ = traceShow (result) ()
  -- print $ maybe 0 snd $ IM.lookupMax result
  print $ result VU.! pred n
