...
main = do
  [n] <- getLineIntList
  !xs <- getLineIntVec
  let !csums = VU.scanl' (+) (0 :: Int) xs

  let result = runSTUArray $ do
        -- dp ! (l, r): minimum cost for merging span [l, r]
        dp <- newArray ((0, 0), (pred n, pred n)) (1000000000000 :: Int)

        -- len == 1 (REMARK: allow range with length one)
        forM_ [0 .. pred n] $ \i -> do
          -- no merge no cost
          writeArray dp (i, i) 0
        ...
        forM_ [2 .. n] $ \len -> do
          forM_ [0 .. n - len] $ \l -> do
            let !r = pred $ l + len
            let !vSpan = csums VU.! succ r - csums VU.! l

            -- find the minimum merge
            vs <- forM [l .. pred r] $
              \m -> do
                x1 <- readArray dp (l, m)
                x2 <- readArray dp (succ m, r)
                return $ x1 + x2 + vSpan

            writeArray dp (l, r) $ minimum vs

        return dp

  print $ result ! (0, pred n)
