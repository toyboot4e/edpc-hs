...
main = do
  !nVerts <- readLn @Int
  !edges <- concatMap (\a -> [a, swap a]) <$> replicateM (pred nVerts) (both pred <$> ints2)

  let !tree = accumArray @Array (flip (:)) [] (0, pred nVerts) edges
  let !_ = dbg $ scanTreeVU tree 0 (modInt 1, modInt 1) (\(!b1, !w1) (!b2, !w2) -> (w2 * b1, (b2 + w2) * w1))

  let !result = foldTree tree 0 (modInt 1, modInt 1) (\(!b1, !w1) (!b2, !w2) -> (w2 * b1, (b2 + w2) * w1))
