...
-- -- | [DAG] BFS for finding the longest path from one vertex to any other
-- bfsLogestPath :: Graph -> Int -> Int
-- bfsLogestPath !graph !start = inner 0 IM.empty (IS.singleton start)
--   where
--     inner :: Int -> IM.IntMap Int -> IS.IntSet -> Int
--     inner !depth !vis !vs
--       | IS.null vs = depth
--       | otherwise = inner (succ depth) vis' vs'
--       where
--         vis' = IM.union vis $! IM.fromSet (const depth) vs
--         vs' = IS.fromList $! concatMap (graph !) (IS.toList vs)
...
main = do
  ...
  !input <- replicateM nEdges (tuple2 . map pred <$> getLineIntList)
  let !graph = accumArray @Array (flip (:)) [] (0, pred nVerts) input
  let !verts = G.topSort graph

  !dp <- VUM.replicate nVerts (0 :: Int)

  let action = foldM stepM s0m verts
      s0m = 0 :: Int
      stepM !accMax !vert = do
        !d <- VUM.read dp vert
        let !vs = graph ! vert
        forM_ vs $ \v -> do
          VUM.modify dp (max (succ d)) v
        return $ max accMax $ if null vs then 0 else succ d
