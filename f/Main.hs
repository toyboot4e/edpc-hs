...
main = do
  ...
  let !result = runSTUArray $ do
        !dp <- newArray ((0, 0), (ls, lt)) (minBound @Int)

        forM_ (range ((0, 0), (ls, 0))) $ \ !i -> do
          writeArray dp i 0

        forM_ (range ((0, 0), (0, lt))) $ \ !i -> do
          writeArray dp i 0

        forM_ [2 .. ls + lt] $ \ !lst -> do
          forM_ [max 1 (lst - lt) .. min ls (pred lst)] $ \ !is -> do
            let !it = lst - is

            !v1 <- readArray dp (pred is, it)
            !v2 <- readArray dp (is, pred it)
            !v3 <- do
              if BS.index s (pred is) == BS.index t (pred it)
                then (+ 1) <$> readArray dp (pred is, pred it)
                else return $ minBound @Int

            -- let !_ = traceShow ((is, it), [v1, v2, v3]) ()
            writeArray dp (is, it) (v1 `max` v2 `max` v3)

        return dp

  -- let !_ = traceMat2D result (0, ls) (0, lt)

  let !lString = result ! (ls, lt)

  let !trackback = thd3 $ until p step s0
      s0 = ((ls, lt), lString, [])
      p ((0, _), _, _) = True
      p ((_, 0), _, _) = True
      p ((_, _), 0, _) = True
      p _ = False
      step ((!is, !it), !n, !acc) = case i of
        0 ->
          let !_ = traceShow (is, it, BS.index s (pred is), acc) () in
          ((pred is, pred it), pred n, BS.index s (pred is) : acc)
        1 -> ((pred is, it), n, acc)
        2 -> ((is, pred it), n, acc)
        _ -> error "unreachable!"
        where
          !i =
            if BS.index s (pred is) == BS.index t (pred it)
              then 0
              else if' (result ! (pred is, it) > result ! (is, pred it)) 1 2
