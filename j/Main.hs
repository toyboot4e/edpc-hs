...
solve :: UArray Int Int -> UArray (Int, Int, Int) Double
solve !arr = runSTUArray $ do
  let !largeEnough = sum $ tail $ elems arr
  let !n = sum $ elems arr

  !dp <- newArray ((-1, -1, -1), (largeEnough, largeEnough, largeEnough)) (0.0 :: Double)

  forM_ (range ((0, 0, 0), (pred largeEnough, pred largeEnough, pred largeEnough))) $ \(i3, i2, i1) -> do
    !e1 <- (*) (intToDouble i1 / intToDouble n) <$> readArray dp (i1 - 1, i2, i3)
    !e2 <- (*) (intToDouble i2 / intToDouble n) <$> readArray dp (i1 + 1, i2 - 1, i3)
    !e3 <- (*) (intToDouble i3 / intToDouble n) <$> readArray dp (i1, i2 + 1, i3 - 1)

    let !i0 = n - (i1 + i2 + i3)
    let !e =
          if i0 == n
            then 0.0
            else (1.0 + e1 + e2 + e3) / (1.0 - intToDouble i0 / intToDouble n)

    let !_ = traceShow (e, (e1, e2, e3), (i0, n, 1.0 - intToDouble i0 / intToDouble n)) ()
    writeArray dp (i1, i2, i3) e
...
main = do
  [n] <- getLineIntList
  !input <- map (,1) <$> getLineIntList

  let !count = accumArray @UArray (+) (0 :: Int) (0, 3) input
  let !result = solve count

  -- let !_ = traceShow ("count", count) ()
  -- let !_ = traceShow (result) ()

  print $ result ! (count ! 1, count ! 2, count ! 3)
