...
main = do
  ...
  let result = VU.foldl' step s0 $ VU.tail input
      s0 :: VU.Vector Int
      s0 = VU.generate (succ k) (\x -> if x <= VU.head input then 1 else 0)
      step :: VU.Vector Int -> Int -> VU.Vector Int
      step !vec !x = VU.generate (succ k) f
        where
          f !i = getRange (i - x - 1, i) `rem` modulus
          !csums = VU.scanl1' (+) vec
          -- !_ = traceShow (vec) ()
          getRange (!l, !r) = csums VU.! r - if l < 0 then 0 else csums VU.! max 0 l
