...
main = do
  ...
  -- TODO: Is it nonsense to seperate `acc0` from `op0`?
  let !result = foldTreeAll tree opm acc0
        where
          !opm = OperatorMonoid op0 mapp mact
            where
              !op0 = (-88888, 1)
              mapp (!_, !op1) (!_, !op2) = (-99999, op1 * op2 `rem` m)
              mact (!acc1, !_) (!_, !op2) = (acc1 * op2 `rem` m, (1 + acc1 * op2) `rem` m)
          !acc0 = (1, 2)

  let !_ = dbg (result)

  VU.forM_ result $ print . fst
