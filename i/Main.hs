...
main = do
  [!n] <- getLineIntList
  !xs <- VU.fromList . map (read @Double) . words <$> getLine

  let !vec = VU.foldl' step s0 xs
      -- acc VU.! n: probability of n coins are showing their front face
      s0 = VU.generate (n `div` 2 + 1) (\case 0 -> 1.0 ; _ -> 0.0 :: Double)
      step !acc !x = VU.generate (n `div` 2 + 1) f
        where
         f 0 = acc VU.! 0 * (1.0 - x)
         f i = acc VU.! pred i * x + acc VU.! i * (1.0 - x)

  let !_ = traceShow (vec) ()
  print $ 1.0 - VU.sum vec
