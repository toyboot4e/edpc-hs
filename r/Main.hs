...
foldTreeAll :: VU.Unbox m => (OperatorMonoid m m) -> m -> Array Int [Int] -> (VU.Vector m)
foldTreeAll (OperatorMonoid !op0 !mapp !mact) !acc0 !tree =
...
mulMatMod :: UArray (Int, Int) Int -> UArray (Int, Int) Int -> UArray (Int, Int) Int
mulMatMod a b =
  listArray @UArray
    ((i0, k0), (ix, kx))
...
main = do
  ...
  !mat <- listArray @UArray ((0, 0), (pred n, pred n)) . concat <$> replicateM n ints

  let !ident = accumArray @UArray (+) (0 :: Int) bounds_ $ map (\i -> ((i, i), 1)) [0 .. pred n]
  let !matVec = newDoubling mat (\m -> m `mulMatMod` m)
  let !matK = foldl' mulMatMod ident $ map (matVec V.!) $ filter (testBit k) [0 .. 62]

  print $ sum . map modInt $ elems matK
