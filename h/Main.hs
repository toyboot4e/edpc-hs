...
main = do
  ...
  let !bounds_ = ((0, 0), (pred h, pred w))
  let !blocks = listArray @UArray bounds_ input
  let !modulus = 1_000_000_007

  let !result = runSTUArray $ do
        !dp <- newArray bounds_ (0 :: Int)
        writeArray dp (0, 0) 1
        ...
        forM_ (range bounds_) $ \i -> do
          unless (blocks ! i) $ do
            ...
            let !i1 = first succ i
            when (inRange bounds_ i1) $ do
              modifyArray dp ((`rem` modulus) . (+ v)) i1

            let !i2 = second succ i
            when (inRange bounds_ i2) $ do
              modifyArray dp ((`rem` modulus) . (+ v)) i2

        return dp

  print $ result ! (snd bounds_)
